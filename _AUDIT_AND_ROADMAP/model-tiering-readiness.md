...
3. **Listing photo licensing** — IDX/MLS display rules and broker-feed photo reuse in agent
   marketing (can a user's campaign legally embed feed photos off-platform? watermark/attribution
   duties?) → determines whether the product ships feed photos in user sends or requires
   user-uploaded photos for send (build free / SEND is the paywall interacts here).
