...
- [ ] §D1 — MCP `swfl_fetch` zip fan-out: `resolveLocation` + `assembleLocationDossier` for zip/location params (no explicit `report_id`)
- [ ] §D3 — search box (any input → redirect) + identity header card + grain chips + did-you-mean; generalize `/r/zip-report/[zip]/page.tsx` off `assembleLocationDossier`
