...
9. A push is not live: brain rebuild via `OPERATOR_APPROVED_PAID_RUN=1 node scripts/dispatch-rebuild.mjs <brain-id> --reason "<decree>"` (never `master --force`); deploys land on Vercel. Say what still has to run.
