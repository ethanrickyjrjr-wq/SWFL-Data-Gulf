...
// ── Edge transitivization ─────────────────────────────────────────────────────
// app_component and lib_module nodes are generated by graphify-app-nodes but
// have no RENDERABLE_TYPES entry — the ops /graph page has no layer for them.
// When those intermediaries are dropped, any component/hook/etc. reachable ONLY
// through them loses all incoming edges and floats on the graph.
// Fix: before filtering nodes, re-wire edges A→dropped→B as direct A→B edges.
// Two passes handles 2-hop paths (lib_module → app_component → component).
const droppedTypeIds = new Set(
  graph.nodes.filter((n) => !RENDERABLE_TYPES.has(n.type)).map((n) => n.id),
);
if (droppedTypeIds.size) {
  for (let pass = 0; pass < 2; pass++) {
    const byTarget = new Map();
    const bySource = new Map();
    for (const e of graph.edges) {
      if (droppedTypeIds.has(e.target)) {
        if (!byTarget.has(e.target)) byTarget.set(e.target, []);
        byTarget.get(e.target).push(e);
      }
      if (droppedTypeIds.has(e.source)) {
        if (!bySource.has(e.source)) bySource.set(e.source, []);
        bySource.get(e.source).push(e);
      }
    }
    const existingKeys = new Set(graph.edges.map((e) => `${e.source}→${e.target}`));
    const newEdges = [];
    for (const [droppedId, incoming] of byTarget) {
      const outgoing = bySource.get(droppedId) ?? [];
      for (const inc of incoming) {
        for (const out of outgoing) {
          const key = `${inc.source}→${out.target}`;
          if (!existingKeys.has(key) && inc.source !== out.target) {
            existingKeys.add(key);
            newEdges.push({ source: inc.source, target: out.target, relation: out.relation });
          }
        }
      }
    }
