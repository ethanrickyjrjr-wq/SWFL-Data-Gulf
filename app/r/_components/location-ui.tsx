  ...
  return (
    <form action={action} method="get" role="search" className="flex w-full gap-2">
      <input
        ...
        autoComplete="off"
        autoFocus={autoFocus}
        className="min-w-0 flex-1 rounded-lg border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-gray-500 focus:border-gulf-teal/60 focus:outline-none focus:ring-1 focus:ring-gulf-teal/40"
      />
      <button
        type="submit"
...
function CardSourceLink({ url, label }: { url: string; label: string }) {
  ...
  if (!c.linkable || !c.href) {
    return (
      <span className="text-gulf-teal" title={c.label}>
        {c.label}
      </span>
    );
  }
  return (
    <a
      ...
      className="text-gulf-teal underline decoration-gulf-teal/40 underline-offset-2 hover:decoration-gulf-teal"
    >
      {c.label}
    </a>
  );
    ...
    <section className="mt-10">
      ...
      <div className="mt-4 space-y-3">
        {lines.map((l) => (
          ...
          >
            <p className="text-xs font-medium uppercase tracking-wider text-gulf-teal">
              {l.coverage_label}
            </p>
  ...
  return (
    <section className="mt-8 rounded-xl glass-card-modern border border-white/10 px-6 py-8 text-center">
      ...
      <p className="mt-4 text-xs text-gray-600">
        Examples:{" "}
        <Link href="/r/search?q=33931" className="text-gulf-teal hover:underline">
          33931
        </Link>{" "}
        ·{" "}
        <Link href="/r/search?q=Naples" className="text-gulf-teal hover:underline">
          Naples
        </Link>{" "}
        ·{" "}
        <Link href="/r/search?q=Lee%20County" className="text-gulf-teal hover:underline">
          Lee County
        </Link>
      </p>
    </section>
