        ...
        <ul className="flex flex-col gap-2">
          {alerts.map((a) => (
            <li key={a.id}>
              <Link
                href={`/alerts/${a.id}`}
                className={`flex items-center justify-between rounded-xl border px-4 py-3 transition-colors hover:border-gulf-teal/40 ${
                  a.read_at
                    ? "border-white/10 bg-[#0d1e2b]/60"
                    : "border-gulf-teal/30 bg-[#0d1e2b]/90"
                }`}
              >
                ...
                <span className="flex flex-col items-end gap-1">
                  ...
                  <span
                    className={`text-[10px] uppercase tracking-wide ${
                      a.answer_sent ? "text-gulf-teal" : "text-amber-400"
                    }`}
                  >
