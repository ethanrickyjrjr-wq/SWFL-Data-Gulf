...
const BTN =
  "rounded-full border border-[#0a8078]/60 px-3 py-1.5 text-xs font-medium text-[#0a8078] transition-colors hover:bg-[#0a8078]/10 disabled:opacity-50";
const BTN_CONFIRM =
  "rounded-full bg-[#0a8078] px-4 py-1.5 text-xs font-semibold text-navy-dark transition-opacity hover:opacity-90 disabled:opacity-50";
const BTN_GHOST =
...
export function SendWeeklyHandle({ deliverableId, projectId, scopeKind, scopeValue }: Props) {
  ...
  if (step.name === "success") {
    return (
      <div className="mt-2 rounded-lg border border-[#0a8078]/30 bg-[#0a8078]/10 px-3 py-2 text-xs text-[#0a8078]">
        ✓ {step.summary}
      </div>
    );
  }
  ...
  if (step.name === "idle" && schedule) {
    ...
    return (
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <span className="rounded-full border border-[#0a8078]/30 bg-[#0a8078]/10 px-3 py-1.5 text-xs text-[#0a8078]">
          {isPaused ? "⏸ Paused — " : "📧 "}
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          {DAYS.slice(1, 6).map((d, i) => (
            <button
              ...
              className={
                dayOfWeek === i + 1
                  ? "rounded-full bg-[#0a8078] px-3 py-1.5 text-xs font-semibold text-navy-dark"
                  : BTN
              }
            >
              {d}
  ...
  if (step.name === "confirm") {
    ...
    return (
      <div className="mt-2 rounded-lg border border-[#0a8078]/30 bg-[#0a8078]/5 px-3 py-3">
