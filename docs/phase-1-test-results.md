...
- [x] In Claude (Pattern B - Project Custom Instructions):
  ...
  - Routing check: **SUCCESS.** Claude answered franchise question from master's charge-off fact, then autonomously fetched `cre-swfl` sub-brain for the corridor list. "I need to fetch the cre-swfl sub-brain..." recorded in transcript.
...
- **f004 Ambiguity (Franchise):** Deterministic fact `f004` uses "total loans" as denominator (e.g. 1/2 charged off). Claude correctly interpreted this but inferred the remainder was "survived/successful". This is an over-claim (remainder could be active). **FIX PENDING:** Refinery logic must explicitly label resolved vs active loans or use resolved loans as the primary denominator to prevent LLM inference errors.
