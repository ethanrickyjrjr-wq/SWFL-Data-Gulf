...
- **Full-site white-label — the whole SWFL site rebranded as the customer's own (operator note, 2026-06-21).** Beyond what already ships — deliverables (`/p/*`) carry zero SWFL chrome, projects brand from the user's profile, and the email funnel auto-populates a prospect's colors+logo into their account profile (`persistClaimBrandToProfile`) — one day we let a customer who wants it run the ENTIRE site experience under their own brand (their colors/logo on every surface, SWFL chrome gone), as a product they use as their own. **Gate: explicitly NOT anytime soon (operator decree).** Per-deliverable/per-project/funnel branding proven in production first, plus the same multi-tenant prereqs (auth, namespacing, billing, theming-as-data) that gate the BYO multi-tenant overlay above. This is the someday extension of today's branding, NOT a near-term build — do not propose it as one.
