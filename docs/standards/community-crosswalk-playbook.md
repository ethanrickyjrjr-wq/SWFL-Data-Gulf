...
- [x] Lee "Planned Developments" ingested into `data_lake` — **BUILT + LIVE 08/28/2026**: `ingest/pipelines/lee_planned_developments/` → `data_lake.lee_planned_developments`, 1,627 rows (1,629 features − 2 empty names), WGS84 via `outSR=4326` server reprojection. Registry entry + quarterly GHA wrapper shipped same PR. Spec: `docs/superpowers/specs/2026-08-12-community-crosswalk-design.md`.
- [x] The actual parcel→PD spatial join — **BUILT + LIVE 08/28/2026**: `spatial_join.py` (DuckDB ST_Contains, no new point-in-polygon code) → `data_lake.parcel_community_pd`: 104,911 assignments / 409 communities; 96,679 servable across 401 after excluding ambiguous + Sketched/"Bad Legal" (`parcel_community_pd_summary_v`). The parcel-coordinate blocker was cleared via LeePA FabricParcels `Latitude`/`Longitude` (`leepa_parcels.latitude/longitude`, 99.80% coverage). **CPD correction:** the playbook's "PUD/RPD/MPD/CPD/etc" wording above is WRONG about CPD — Commercial Planned Development is the layer's largest category (749) and is never joinable; the join uses a named residential allowlist (RPD, PUD, MPD, MHPD, RVPD, CFPD-RPD, CPD-RPD, MHPD-RVPD). Consumers: `lib/listings/community-identity.ts` (email path, silence rules) + communities-swfl pack metric.
- [ ] Collier County's equivalent layer — searched, not yet confirmed as of this writing (`hub-collierbcc.opendata.arcgis.com` is the lead). **Do not re-run this search from zero — check whether it landed before searching again.** (Still open 08/28/2026; Collier also holds no parcel coordinates in our lake — both halves needed.)
