...
| **Comp set — sold candidates for ONE subject home** | `lee_comp_sales_v` 🟡 (Lee only; `leepa_parcels` sale + `lee_parcels` FDOR characteristics, joined on the 07/19 strap crosswalk) → ranked by `lib/assistant/comp-rank.ts` | no single brain (answer-engine path) | the vendor `/nearby-home-values` feed is NOT a sold-comp root — it carries **no sale date at all** (AVM `estimateDate` is "not a sale") · **no Collier equivalent exists** — do not serve Collier comps from this — **T9** |
