...
- FUTURE TYPED ROOTS (🔴, Step 3 of the playbook, each behind the brain-first gate): per-parcel tax/valuation series (`tax_history[]` — the intended fix for check `should_i_sell_property_tax_source`'s false premise) · per-property permits (`building_permits[]` — serves `permits_spine_thin_collier_missing`, incl. Collier) · full listing event history (`property_history[]` — `days_after_listed`, `last_status_change_date`, `source_name`). One-to-many: NEVER onto `listing_state`.
