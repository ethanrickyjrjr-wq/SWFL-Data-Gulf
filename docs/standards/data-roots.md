...
| **Sale DATE — grain + recency** | month grain today, everywhere: `leepa_parcels.last_sale_date` (Lee) · `sale_mo1`+`sale_yr1` integers (`lee_parcels`/`collier_parcels`, FDOR) | properties-lee-value · properties-collier-value | `lee_deed_official_records.record_date` 🟡 is the ONLY day-grain sale date and it is **NO LONGER EMPTY — first real data 08/12/2026**: 28,186 rows, 07/13–08/11/2026, 22 business days, all 32 doc types (5,353 DEED, of which 3,229 arm's-length >$100 and 2,124 nominal ≤$100). Cite the SPAN, never imply a continuous series — FETCH is still manual (Akamai), so history advances only on a human drop. **Parcel join is BROKEN**: `parcel_strap` (`0-44-27-02-00012.0130`) does not match `lee_parcels.state_parcel_id` (`C46-000-502-7399-4`), 0 rows on a direct join, live 08/12/2026 — **T10**. **NEW ROOT 08/12/2026 — `collier_official_records.record_date` 🟡**: Collier's Clerk of Courts feed (`cor.collierclerk.com`), day-grain, ALL 37 doc types, fully automated daily cron (no Akamai wall found, unlike Lee — see the liveness probe). **Not a sale-price root** — this search UI carries no consideration/sale-price column at all (unlike Lee's), so it cannot replicate Lee's cash-vs-financed pattern or feed a Collier sold-median; it is document-activity metadata (grantor/grantee, doc type, book/page, legal description) only. Parcel-join coverage against `collier_parcels` is unmeasured. |
