...
| **Price cut — EVENT (per-listing)** | **TWO LANES, different questions — DISAMBIGUATED 08/04/2026, never blended and never summed together:** `listing_transitions.price_delta` 🟢 = OUR FORWARD-ONLY sweep (every cut since we started watching that listing; complete inside the window, blind before it) · `steadyapi_listing_events_v.price_change` 🟡 = the VENDOR'S BACKWARD history (the whole past cut trail incl. the original ask, but only for properties we probed) | no single brain (rollups `listing_pulse_daily`, `listing_transitions_recent_zip_stats`) | a "total price cut" off the sweep UNDERSTATES any listing older than our watch; the same figure off the vendor lane is listing-scoped and can never be an area statistic — **T11** |
| **Listing outcomes — training panel (weekly)** | `listing_week` 🟡 (labels one week in arrears; last completed week censored) | no brain (feeds sell-odds model, spec 07/19) | never read as a served count — `listing_active_stats` is the active-count root |
...
| **Active inventory — for-sale HOMES** | `listing_active_stats.listing_count` 🟢 **RATIFIED — operator decree 07/19** (homes-only, Lee/Collier; add `zip_code IS NOT NULL` to skip region rollup). Consumers repointed 07/19: zip-report card + page AI metrics + weekly-read/zip-seed cards (`lib/zip-report/candidates.ts`) + activation email (`lib/email/activation/snapshot.ts`) | active-listings-swfl | `housing_by_zip.inventory` (Redfin END-OF-MONTH — monthly trend cross-check ONLY, never a "now" count; demoted to the rail) · `listing_momentum_stats.active_listing_count` (all-types, +~7,300 land, ~3.2×) · `active_listings_residential*` (dead) |
...
| **Comp set — sold candidates for ONE subject home** | `lee_comp_sales_v` 🟡 (Lee only, RESIDENTIAL only since 08/02/2026 — dor_uc 001/002/004/005/007/008; `leepa_parcels` sale + `lee_parcels` FDOR characteristics on the 07/19 strap crosswalk + **beds/baths from `leepa_comparable_sales` (LeePA layer 23) via folioid lateral join, 1–10 sanity ceiling** — docs/sql/20260802_lee_comp_sales_v_beds_baths.sql; also the FREE baths fallback for the email subject path, `lib/listings/resolve-subject.ts` withBaths, unique-address-match only; **+ `pool` — THE ONE POOL ROOT, wired 08/04/2026**: 108,848 source rows 100% filled, ingested 07/22 and unread for 13 days, normalised in exactly one function `poolFromSource` (`lib/assistant/comp-source-lake.ts`), docs/sql/20260804_lee_comp_sales_v_pool.sql) → ranked by `lib/assistant/comp-rank.ts` | no single brain (answer-engine path) | **Collier has NO pool source at all** — check `collier_pool_no_source`; probe Collier's own GIS before paying a vendor · a permit of type `'Pool'` (`steadyapi_property_permits_v`) is an EVENT, not pool ownership — the two banked pool candidates (`amenities_listing_detail_hoa_lift`, `amenities_apify_maps_ground_truth`) may not ship without retiring this root first · the vendor `/nearby-home-values` feed is NOT a sold-comp root — it carries **no sale date at all** (AVM `estimateDate` is "not a sale") · **no Collier equivalent exists** — do not serve Collier comps from this — **T9** |
| **Sale DATE — grain + recency** | month grain today, everywhere: `leepa_parcels.last_sale_date` (Lee) · `sale_mo1`+`sale_yr1` integers (`lee_parcels`/`collier_parcels`, FDOR) | properties-lee-value · properties-collier-value | `lee_deed_official_records.record_date` 🟡 is the ONLY day-grain sale date and it is **NO LONGER EMPTY — first real data 08/12/2026**: 28,186 rows, 07/13–08/11/2026, 22 business days, all 32 doc types (5,353 DEED, of which 3,229 arm's-length >$100 and 2,124 nominal ≤$100). Cite the SPAN, never imply a continuous series — FETCH is still manual (Akamai), so history advances only on a human drop. **Parcel join is FIXED 08/12/2026 — it was never broken, it was measured against the wrong column.** `parcel_strap` does not match `lee_parcels.state_parcel_id` (that 0-row result was correct), but it DOES match `lee_parcels.**parcel_id**` once separators are stripped and the section field is zero-padded — both are the 17-char Lee STRAP. **ROOT: `data_lake.lee_records_addressed_v`** (live, `docs/sql/20260812_lee_records_addressed_v.sql`) — **10,461 recorded documents resolved to a STREET ADDRESS across 31 doc types**, 07/13–08/11/2026: DEED 4,535 (85% of all 5,353 deeds), MORTGAGE 1,846, NOTICE OF COMMENCEMENT 1,278, LIEN 313, PROBATE 120. `sale_price_usd` is NULL off a DEED **by construction** (consideration is non-null on 100% of rows incl. death certificates, so the >$100 floor is meaningless elsewhere). Dedupe on (record_date, strap17) before counting sales — two deeds can share one address on one day. Evidence: `_RESEARCH/data-and-ingest/2026-08-12-deed-parcel-strap-join-fix.md`. **NEW ROOT 08/12/2026 — `collier_official_records.record_date` 🟡**: Collier's Clerk of Courts feed (`cor.collierclerk.com`), day-grain, ALL 37 doc types, fully automated daily cron (no Akamai wall found, unlike Lee — see the liveness probe). **Not a sale-price root** — this search UI carries no consideration/sale-price column at all (unlike Lee's), so it cannot replicate Lee's cash-vs-financed pattern or feed a Collier sold-median; it is document-activity metadata (grantor/grantee, doc type, book/page, legal description) only. Parcel-join coverage against `collier_parcels` is unmeasured. |
| **Permits — per-PROPERTY (listing-scoped)** | `steadyapi_property_permits` (TABLE) 🟡 — 79,281 permits / 12,946 properties, live-probed 08/04/2026. **✅ SECOND PARSE ELIMINATED 08/04/2026** (check `steadyapi_three_families_table_vs_view_duplicate`). Two OBJECTS remain and that is the INTENDED shape — table + read view is exactly [`data-consolidation-plan.md`](./data-consolidation-plan.md). What was wrong was two **parses**: the TABLE shipped 08/03 (handoff `2026-08-03-steadyapi-step3-sonnet-handoff.md`; this file still said 🔴 not-built until 08/04) and the VIEW `steadyapi_property_permits_v` was built 08/04 re-parsing the same raw bytes at the **identical row count**, unaware of it. The view now reads the TABLE (`from data_lake.steadyapi_property_permits`, verified live via `pg_get_viewdef`, 79,281 = 79,281): TABLE keeps the unique key + pkey, VIEW keeps the guards (`effective_date` NULLed outside [1900-01-01, today], raw kept). **⚠️ ONE HONEST LOSS:** the table does not store the vendor's literal date string, so `effective_date_raw` is now that date as ISO text, not "Mar 8, 2021" — restore it in the PARSER if needed, never by re-parsing (check `steadyapi_permits_vendor_date_string_not_stored`). View `docs/sql/20260804_steadyapi_property_permits_v.sql`, typed reader `lib/listings/property-permits.ts` (reads the VIEW; **has no caller**) | no brain yet — check `steadyapi_listing_events_shipped_needs_consumers` | **NEVER an area permit statistic** — `lee_building_permits` / `collier_building_permits` stay the full-book area lane; a Steady row exists only because that property was listed or sold · `permit_type = 'Pool'` is a permit EVENT, not pool ownership — do not make it a second pool root · vendor duplicates measured 15.6% on the loose key vs **1.1% byte-identical** (only the byte-identical kind is collapsed; check `steadyapi_permits_duplicate_rows`) · `effective_date` is the human string "Mar 8, 2021" on 79,281 of 79,281 rows — **T12** |
| **Tax / assessment history — per-PROPERTY year series** | `steadyapi_tax_history` (TABLE) 🟢 — 273,051 year-rows / 16,514 properties, live 08/03/2026, `(property_id, tax_year)` unique. **✅ SECOND PARSE ELIMINATED 08/04/2026** (`steadyapi_three_families_table_vs_view_duplicate`): the VIEW `steadyapi_tax_history_v` was built 08/04 re-parsing the same bytes at the identical count; it now reads the TABLE (`from data_lake.steadyapi_tax_history`, verified live via `pg_get_viewdef`, 273,051 = 273,051). Two objects is the intended shape — one parse, in the parser; the view types and guards (future-year rejection) only. **The TABLE is the one with a live consumer** — `listing_recent_tax_paid_stats` → `refinery/sources/active-listings-residential-source.mts` → active-listings-swfl (`median_annual_tax_swfl`). View `docs/sql/20260804_steadyapi_tax_history_v.sql`, typed reader `lib/listings/property-tax-history.ts` (reads the VIEW; **has no caller**) | active-listings-swfl (via the rollup, TABLE side only) | **NOT a valuation authority** — full-book assessed/market value stays `leepa_parcels` (Lee) / `collier_parcels` (Collier, which IS the FDOR pull, so **Collier is already covered** — the "Steady covers Collier where LEEPA doesn't" shorthand is WRONG) · `tax_amount` is parsed and live but **NOT cleared for user-facing serving** until validated against a real county bill (`TAX_AMOUNT_NOT_CLEARED_FOR_SERVING` rides on every summary object; check `steadyapi_tax_amount_validation_owed`) · key presence ≠ value presence: `assessment_building` 22.0% and `assessment_land` 19.0% populated — rendering an absent split as `$0` is a fabricated figure — **T12** |
| **Listing EVENT history — per-property** | `steadyapi_listing_events` (TABLE) 🟢 — 235,383 events / 17,859 properties, live 08/03/2026, `dedupe_key`. **✅ SECOND PARSE ELIMINATED 08/04/2026 — the view no longer re-parses.** (Two OBJECTS remain by design; two PARSES was the defect.) `steadyapi_listing_events_v` was built 08/04 as a SECOND parse of the same array at the identical count; it is now a **READ LAYER over this table** (`from data_lake.steadyapi_listing_events`, verified live via `pg_get_viewdef`). One parse, in the parser; the view types and guards only. That is [`data-consolidation-plan.md`](./data-consolidation-plan.md) as written — "only ingest writes base tables; only root views read them; only consumers read root views" — and it keeps BOTH halves: the TABLE's live consumer (`listing_recent_price_cuts_stats` → active-listings-swfl, `recent_price_cuts_count_swfl`) and the VIEW's guards (`is_rental`, `price_is_zero_sentinel`, the [1900-01-01, today] date floor, and a `price_change_pct` that counts a value as a percentage ONLY when it carries a '%' and no '$' — 42,633 of the 44,896 present values, with the 2,263 dollar-shaped strings left null instead of printing "-900%" off a $900 cut, plus `price_change_pct_raw` and a `price_change_pct_conflict` flag on the 183 sign-contradiction rows). Live after the rewrite: 235,383 rows · 42,633 percentages · 17,022 rent events · 31,217 public-record rows · 124 dates refused. **The TABLE remains the root; the view is never a root.** View `docs/sql/20260804_steadyapi_listing_events_v.sql`, typed reader `lib/listings/listing-events.ts` (reads the VIEW; **has no caller yet**). Families B (`steadyapi_tax_history`) and C (`steadyapi_property_permits`) were fixed the same way in the same pass — all three verified live. Either way this is the SOLE source for per-event price-change amounts, the originating MLS board, and status-change timestamps | active-listings-swfl (via the rollup, TABLE side only) | this is the BACKWARD vendor lane of the Price-cut EVENT row above — never blended with the forward sweep (**T11**) · `listing_dom` STAYS the DOM root; `days_after_listed` here is a VALIDATION column (and is the human string "111 days") · rent events share the array — 17,022 across 3,696 properties — filter `is_rental` or a $7,500/mo rent lands in a sale-price average · `price = 0` is a sentinel on 45.9% of "Listing removed" (NULLed, flagged) while `price_change = 0` is REAL ("no change") · a null `listing_id` means "public record", not a broken row — **T12** |
| **Community / subdivision grain — names, home counts, values** | `neighborhood_stats` 🟢 (annual aggregate; one row per county+subdivision: `home_count`, count-by-type, `median_just_value`) ← `parcel_subdivision_v` ← `lee_parcels`+`collier_parcels` | communities-swfl | amenity roots LIVE 08/03/2026: `community_profiles` 🟡 (IN-GATE facts — golf structure/HOA range/gated/amenity flags; **81 rows, re-counted live 08/05/2026** — an earlier "69 rows" line here was stale; coverage thin — scale-out check `amenities_profiles_scale_out`. **CONSUMER WIRED 08/05/2026:** `lib/listings/community-inside-the-gate.ts` `resolveInsideTheGate` is the read root for the address-spine emails, joined on the SAME canonical community slug `neighborhood_stats` uses so the two layers cannot drift onto different names for one place, **with a slugified-label fallback because all 81 rows were written with `community_slug` = their own slugified label and an alias-map-only key would have found approximately none of them.** Every flag is TRUE-ONLY: a `false` and a `null` are BOTH "we do not know" and both stay SILENT — 81 profiles against 20,400 subdivisions makes a miss the NORMAL case, and a miss must never render as "this community has no golf") + `steadyapi_neighborhoods`/`steadyapi_neighborhood_amenities`/`steadyapi_property_neighborhood` 🟡 (AROUND-the-community: vendor neighborhood w/ boundary polygon + centroid, 12 location scores, nearby businesses w/ ratings; property_id→slug_id = the api_feed listings↔community pairing edge; `ingest/pipelines/neighborhood_amenities/`; 245 neighborhoods · 16,304 amenities · 19,805 pairings as of 08/03/2026, all from a MANUAL run — the daily 09:30 UTC cron has never landed a scheduled fire, `ENGINE_ENABLED=false` skips it, so this data does NOT refresh itself today; registry entry graduated out of parked and probed 08/03/2026; **CONSUMERS WIRED 08/04/2026** — `lib/listings/neighborhood-amenities.ts` is the listing→community entry point (vendor pairing on `property_id` first, then lat/lon ray-cast against stored boundary polygons via `lib/geo/ray-cast.ts`), feeding every address-spine recipe through `recipes/shared.ts`; `community-info.ts` still owns the typed-NAME lane. Until 08/04 the pairing edge had ZERO readers — Gate 12 in `check-prepush-gate.mjs` now BLOCKS a new `data_lake.*` table that nothing reads. **The amenities are NEARBY businesses in the vendor's 5-mile radius, NOT in-gate amenities** — `community_profiles` is the in-gate root; `neighborhoodAmenitiesSourceLine` carries that prohibition and its wording is test-enforced) — two DISTINCT concept families, never merge · raw `legal_description` grouping · **GRAIN TRAP: city-scale plat rollups (Lehigh Acres = one 29,225-home row) are CITIES, not communities — sanity-check `home_count` (~50–2,000 = community)** |
| **Community IDENTITY — which marketed community a Lee parcel is physically inside (geometry, no name-matching)** | `parcel_community_pd` 🟢 (LIVE 08/28/2026 — first run: 104,911 assignments, 409 communities; **96,679 servable across 401** after excluding 7,429 ambiguous + 803 Sketched/"Bad Legal"; aggregate read root = `parcel_community_pd_summary_v`, never the raw table) ← DuckDB ST_Contains over `lee_planned_developments` 🟢 (all 1,629 county PD polygons, WGS84; Approved+residential-allowlist filter applied at READ and recorded per-row in `assignment_criteria`) × `leepa_parcels.latitude/longitude` (FabricParcels parcel points, 99.80% coverage, backfilled 08/28/2026 via `scripts/backfill_leepa_parcel_coords.py`) | communities-swfl (`parcels_with_pd_community_identity_lee`) | **SCOPE: unincorporated Lee ONLY** (+ legacy incorporated polygons) — Cape Coral/Fort Myers/Bonita Springs NOT fully covered, Collier EXPLICITLY excluded (220,875 parcels, no coordinates) — never frame as "the community answer for Lee." Email-path consumer: `lib/listings/community-identity.ts` `resolveCommunityIdentity` (per-parcel; SILENT for Sketched/"Bad Legal"/ambiguous; scope wording test-enforced), fed by `community-lookup.ts` parcelIds, feeding `resolveInsideTheGate` as the FIRST key so the 3 name-matched profiles (Shadow Wood Preserve, West Bay Club, Wildcat Run) fire from geometry. Spec: `docs/superpowers/specs/2026-08-12-community-crosswalk-design.md`; pipeline `ingest/pipelines/lee_planned_developments/` |

**Traps (nuance the cells can't hold):**
- **T1 — ⚠ SUPERSEDED 08/04/2026. THE FLOOR IS LARGELY GONE — RE-COUNT BEFORE QUOTING ANYTHING BELOW.**
  **Live exact counts, `data_lake.listing_dom`, 08/04/2026:** TOTAL **34,904** · floored **3,079
  (8.8%)** · real **31,825 (91.2%)** · Lee **22,458 of 24,548 real (91.5%)** · Collier **8,202 of
  9,142 real (89.7%)** · `listed_date` present on **31,309**. The backfill the 07/20 note said was
  WIPED has since landed. **Everything in the 07/18–07/20 text below is kept only as history and
  must not be quoted as current** — it says 54.2% floored and Collier 14.0% real, and on 08/04/2026
  a session quoted exactly that into a committed email playbook and had to be corrected by the
  operator. Aggregate DOM is now usable; suppress on coverage only where a specific slice is thin,
  and count that slice live.

- **T1 (HISTORY, 07/18–07/20/2026 — DO NOT QUOTE AS CURRENT) — aggregate DOM is not trustworthy today.** Row-level `listing_dom` is fine; the *aggregate* is censored — ~63% of the active book is a `first_seen` floor (07/18 backfill de-flooring it). A 30-sec "typical DOM" off the aggregate is confidently wrong right now.
...
- **T9 — a comp set is not a sold-price median, and the vendor feed cannot date a sale.** `lee_comp_sales_v` is the *candidate universe* for comparing ONE home, not an area statistic — for "what sold for" use the Value—sold row above. Live-measured 07/22/2026: **387,609 home sales all-time · 17,859 trailing 12 months · 8,999 trailing 6 months · 41 ZIPs**, `living_area_sqft > 0` as the home test (excludes land, per Fannie B4-1.3-08), `last_sale_amount > 1000` to drop $1 quitclaims. **The separate vendor feed has NO sale date field whatsoever** — only an AVM `estimateDate` the vendor labels "not a sale" — so the answer-engine path ranks it on size band + shape only and reports `recencyVerified: false`. Never map an AVM date into a sale date to satisfy a window; that is an invented fact.
- **T10 — every sale date we serve is MONTH grain, and it lags ~7 weeks.** Live-probed 07/22/2026: across all 387,609 rows of `leepa_parcels.last_sale_date` there is **exactly one distinct day-of-month, the 1st** — zero exceptions. Not a normalizer artifact (the `last_sale_date__v_text` variant is NULL on every recent row, so there is no day-precision to recover). Render "May 2026", **never "05/01/2026"** — an exact date asserts precision the source lacks. Recency: newest month is **06/01/2026, 51 days behind**, and that newest month is **thin (745 vs May 1,931, April 2,377)** — it is still filling, so "last 6 months" really means the last ~5 complete months. Dirty floor: 4 rows on 01/01/1900, 2,949 pre-1980, oldest plausible 08/1951. Collier is worse — `sale_mo1`/`sale_yr1` are separate integers, month grain by construction with no date column at all. **The day-grain fix is `lee_deed_official_records` (`record_date`, `consideration_usd`, `parcel_strap`) and as of 08/12/2026 it HOLDS REAL DATA** — 28,186 rows across 07/13–08/11/2026 (22 business days), unfiltered across all 32 doc types. Two live caveats before anyone serves off it: (1) the FETCH is still manual — Akamai blocks every unattended fetch of the Lee Clerk site (crawl4ai, CDP Chromium, curl, curl_cffi w/ Chrome TLS all failed 07/19–07/20/2026), so a human captures each day and the span grows only when they do; (2) ~~the parcel join does not work yet~~ **THE JOIN WORKS — FIXED 08/12/2026, and T10's month-grain complaint is now answerable at DAY grain with a street address.** The 0-row result was real but was measured against `lee_parcels.state_parcel_id`; the correct column is `lee_parcels.parcel_id`, which is the same 17-char STRAP with separators stripped (a fact already recorded 07/18/2026 in `_RESEARCH/audits/2026-07-18-data-consolidation/P1-parcel-consolidation.md` line 63 and never connected to this feed). `parcel_strap` is on 12,399 of 28,186 rows (44%); of the 4,999 strap-carrying DEEDs, **4,535 resolve to a street address (91% of strap-carrying, 85% of all deeds)**. Serve it from `data_lake.lee_records_addressed_v` — sale price + exact recording day + street address, e.g. 08/11/2026 · $350,000 · 2806 SE 17TH AVE. `leepa_parcels.strap` is a second working route (4,563 matched), and **that same key closes the LeePA↔FDOR crosswalk the 07/18 audit called nonexistent — 542,445 of 548,798 rows, 98.8%** (dedupe: 826 duplicate straps on the LeePA side). Queue: `docs/handoff/2026-08-12-lee-deed-data-investigation-queue.md` task 1. **Unpulled ceiling:** LeePA MapServer **layer 23 is literally named "Comparable Sales"** — 108,881 rows carrying `BedRooms`, `Bathrooms`, `YearBuilt`, `GrossArea`, `SalePrice` and `SHAPE` (coordinates), joinable on `FOLIOID` which `leepa_parcels` already has. It would close the bed/bath gap and enable true distance, but it is `SaleYear`+`SaleMonth` — **month grain too, so it does not fix recency**.
- **T11 — a price cut has two honest answers and they disagree BY DESIGN.** Our `listing_transitions.price_delta` is a FORWARD-only sweep: it can only see cuts that happened after we started watching that listing, so on a listing older than our watch a "total cut" is confidently too small. The vendor's `steadyapi_listing_events_v.price_change` is a BACKWARD history: it carries the whole trail back to the original ask (measured 08/04/2026 — the amount is a real number on all 235,383 events), but it exists only for the ~17.9k properties we probed, so it can never roll up to a ZIP or county. **Neither is wrong; they answer different questions.** Label which lane a cut came from, never sum across them, and never let the vendor lane leak into an area share (that root is `listing_momentum_stats.price_reduced_share`, T3).
- **T12 — LISTING-SCOPE LAW: the three Steady per-property roots are not a book.** `steadyapi_property_permits_v`, `steadyapi_tax_history_v`, `steadyapi_listing_events_v` each contain a row ONLY for a property we probed because it was listed or sold. County rolls (`lee_parcels`/`collier_parcels`, `lee_building_permits`/`collier_building_permits`, `leepa_parcels`) are full-book. Any average, count, or share taken over a Steady view is a statistic about *our probe history*, not about a place — 12,946 / 16,514 / 17,859 properties against ~1.1M SWFL parcels. Each typed reader carries a `scopeNote` on every summary object so the caveat cannot be lost in transit; do not strip it, and do not aggregate around it.
...
**Rules (enforced by the gate — see [`data-consolidation-plan.md`](./data-consolidation-plan.md)):**
...
### ⛔ THE APIFY VENDOR RULES — the DATA half. The recipe lives in emails.md.

**➜ Building or editing a comps email? [`emails.md`](./emails.md) §0.6b is THE ONE RECIPE** — set
selection, row contents, block order, type/grid, verification. Operator, 08/04/2026: *"Write the
recipe as the only fucking one for comps."* This block keeps ONLY what belongs to a data catalog:
...
08/04/2026 — `"14503 DOLCE VISTA RD, FORT MYERS, FL 33908"`, radius 0.3, returned 12078 Terraverde
...
email about 8348 Southwindbay Cir, 33908.

**R3 · AN UNWINDOWED ZIP SWEEP CAN NEVER JOIN.** 24 arbitrary sold homes in a ZIP with hundreds of
sales intersected 6 specific comps **0 times, every build, by construction.** Narrow the ZIP to the
comps' own sale window (`compSaleMonthWindows`), then join on address. **No derivable window → buy
NOTHING** — the vendor AVM lane sets `priceDate: null` on every comp, so that lane costs $0.

**R4 · ONE PULL PER SALE MONTH, NOT ONE WIDE WINDOW.** The cap cuts the pull short from the recent
end. Measured: one 4-month window @120 joined **2 of 6**; the same budget split per month @200
joined **5 of 6**. ZIP 33908 alone returns a full 200 for a single month. Lake sale dates are MONTH
GRAIN (`lee_comp_sales_v.sale_month` is always day-of-month 1), so the month is the tightest honest
window — a ±3-day window is not available to us.

**R5 · TWO ADDRESS NORMALIZERS EXIST AND THEY ARE NOT INTERCHANGEABLE.** The wrong one returns
**zero rows and no error** — the single most likely way to lose a session.
· `listingAddressKey("14503 DOLCE VISTA RD","FORT MYERS")` → `"14503 dolce vista rd fort myers"` —
keys the **CACHE TABLE**.
· `compPhotoKey(...)` → `"14503DOLCEVISTARD@FORTMYERS"` — keys the **PHOTO/URL INDEXES**.

**R6 · READ THE CACHE BEFORE YOU PAY, AND MAKE SURE THE WRITE LANDS.** ⚠️ **Postgres 21000** — `ON
CONFLICT DO UPDATE command cannot affect row a second time` — rejects the ENTIRE batch when the
incoming rows repeat one `address_key`, which a 200-record ZIP pull hits constantly. That error was
swallowed, so ~600 paid records landed **zero** rows while the table sat at 20. `dedupeRows` is the
guard; never remove it. **This is how a $29 monthly budget was spent in a week: every build re-bought.**

**R7 · A VENDOR REFUSAL IS NOT AN EMPTY MARKET.** Three identical pulls returned 200, then 101, then
0 — the 0 was HTTP 403 `platform-feature-disabled: Monthly usage hard limit exceeded`. Identical
counts across different windows is the throttling tell; a count under the cap is NOT proof of a small
month.
...
**`full_baths`/`half_baths`** (via `bathsFromRecord`, the ONE conversion root) all ride in on the
same response. This lane read three of them for a week while baths rendered blank in the email.

**WHICH ACTOR ANSWERS WHICH QUESTION**
· `moving_beacon-owner1/realtor-com-property-scraper` (~$0.01/result) — **areas over a date range.**
The one wired lane.
· `one-api/realtor-property-scraper` (~$0.007/result) — a **true per-property lookup**, keyed on
`property_inputs: [<realtor.com detail URL>]`. The lake comp lane carries no detail URL, so it is
only reachable DOWNSTREAM of a dated ZIP pull. **Not wired.**
...
- **`data_lake.apify_property_records`** 🟢 — root: **the full realtor.com record for one address**,
  ...
  `fetchCachedRecords(keys, maxAgeDays)`, **plus `fetchCachedRecordLoose(street, city, days)` as a
  SECONDARY key** (see the key-drift note below).
  · **`property_url` IS THE LISTING-LINK ROOT** — the vendor's own url string, stored verbatim, and
  **26 of 26 rows carry it** (counted live 08/05/2026, the best-filled column on the table). The
  free spine (`listing_state`) has **NO url column of any kind** — all 42 columns read from
  `information_schema` 08/05/2026. Consumer root: `lib/listings/listing-url.ts listingButtonUrl`,
  which returns `null` rather than ever falling back to our own site (playbook §1.8). **Never
  derived from the permalink formula and never fetched to confirm** (operator 08/05/2026: *"we
  aren't fucking scrapping"*).
  · ⚠️ **THE ADDRESS KEY DRIFTS BETWEEN THE TWO FEEDS — measured 08/05/2026.** The daily sweep
  writes `12554 Kellysands Way`; this table writes `12554 Kelly Sands Way`. Same street, same
  house, two spellings, no join. Across all 26 paid rows: **8 join on the exact
  `listingAddressKey`; 5 more join only once spacing is ignored** (McGregor Woods, Kelly Sands,
  Marco Island, Creekside View, Bristol Bnd). `looseAddressKey` (every non-alphanumeric removed)
  is the secondary, tried ONLY after the exact key misses. Safety measured before adoption:
  across **30,655 active listings it produces 25 collisions and every one is the same place
  spelled two ways**, never two different houses — and house number + city both stay in the key.
  · ⚠️ **A CONSUMER MUST PASS THE STREET LINE, NOT THE FULL ADDRESS.** `fillFromPaidRecord` passed
  `facts.address` ("12554 Kellysands Way, Fort Myers, FL 33908") into a key built from the STREET
  column alone, so it missed on every address carrying a comma. Fixed 08/05/2026; the shape of the
  bug is worth remembering because both halves of a join have to normalise the same way.
...
- DATA AVAILABLE, unpulled: **the `/property-tax-history` responses we ALREADY PAY FOR carry 64 field paths; we persist 3** (08/02/2026 four-property live census — `_RESEARCH/data-and-ingest/2026-08-02-property-tax-history-full-scope.md`). Unread families: `tax_history[]` (9-yr per-parcel tax+valuation, covers Collier where LEEPA doesn't) · `building_permits[]` (per-property permit history — should-get #2 since 07/16) · full `property_history[]` (per-event price cuts, `days_after_listed`, `source_name` MLS board, `last_status_change_date`) · `statistics{}`/`meta{}` rollups. **CORRECTION 08/02:** the old claim "only aggregate median_days_on_market exists, not per-listing" is FALSE for this endpoint — `property_history[].days_after_listed` is the vendor's own per-event DOM. Brokerage/agent fields remain a genuine vendor ceiling (zero across all 18 endpoints). Root cause the fields were never landed + build order: `docs/superpowers/plans/2026-08-02-steadyapi-raw-landing-playbook.md` (the `fetch_sold_event` raw-body discard, `extract_api.py:586`).
- ROUTES:
...
### steadyapi_property_history_raw · backfill/probe lane · lane tier-2 (cold landing)
- STATUS: 🟢 **live, 17,875 bodies** — table + code shipped 08/02/2026, paid backfill run (Step 2) completed by 08/03/2026 (newest `fetched_at` 08/03 01:11 UTC). `undated_active_sale` = 0 — the DOM backfill fully de-floored. Nightly cron still disabled (`ENGINE_ENABLED=false`, operator-owed re-enable, check `engine_enabled_off_all_crons_dark`). (`engine_enabled_kill_switch_owed` was a different, already-closed 08/02 check asking the operator to turn the flag OFF — not this one, which is about turning it back ON.)
- ROOT: **IS the root** for raw `/property-tax-history` vendor bodies — `data_lake.steadyapi_property_history_raw (property_id text PK, address_key, county, body jsonb, fetched_at)`, idempotent `ON CONFLICT (property_id) DO UPDATE`. Cold provenance landing, NOT a consumer surface; typed roots parse OUT of it with zero paid calls. **JSON PATH: every family array lives at `body->'body'->'<family>'`, NOT top-level** — `body->'property_history'` returns NULL on every row and reads as empty with no error.
- WHY IT EXISTS: `extract_api.py:586` `fetch_sold_event` discards the raw body at its `return` — the reason 7+ audits (06/30→08/02) re-found 64 fields and shipped 3. Once bodies land here, every future field is a free SQL query instead of an unreachable code change. Full build spec: `docs/superpowers/plans/2026-08-02-steadyapi-raw-landing-playbook.md`.
- **SECOND WRITER + THE ONLY READER — the TS BUILD PATH, wired 08/19/2026.** The playbook fixed the discard on the *ingest* lane only. `lib/listings/steadyapi.ts` `fetchSoldEvent`/`fetchListedDate` hit the SAME endpoint from every email build and chat comp answer, and did the same thing the Python did before 08/02: read three numbers, drop the body. Measured that day: 33 files name this table, **none under `lib/` or `app/`** — the build path neither wrote nor read one row of 18,319. Consequence: a close that rendered in one build was gone from the next the moment the vendor didn't answer, and nothing else could cover it — `lee_comp_sales_v` is MONTH-grain (`sale_month`) and `/nearby-home-values` carries no sale date at all (T9), so this endpoint is the ONLY day-grain recorded close reachable at build time. Write+read root: **`lib/listings/sold-event-store.ts`** (`saveHistoryBody` / `fetchStoredHistory`), called from INSIDE the one fetch root so every lane gets persistence without opting in — same shape as `apify-record-store.ts` inside `apify-comps.ts`. Same table, same `property_id` PK, no second root. Three guards, each tested: (1) `address_key` is written with the parity port `lib/listings/address-key.ts` and OMITTED (never nulled) when the call site doesn't know street+zip, so a build-path re-probe cannot wipe the ingest lane's key; (2) the stored body answers only when the vendor returned NO body — a 200 carrying no Sold event is the source saying there is no sale, and is never overridden from an old copy; (3) every fetched event carries `provenance: "live" | "stored"` + `asOf` (the OBSERVATION date, not the sale date), and the fallback expires at `STORED_FALLBACK_MAX_AGE_DAYS` = 180.
- TYPED ROOTS (Step 3 of the playbook, each behind the brain-first gate) — **AUTHORITY DESIGN LOCKED 08/02/2026 (operator decree: best source per concept, validate what we already hold, listing-scope law; full design = playbook §STEP 3):**
  - `steadyapi_listing_events` (← `property_history[]`) — 🟢 **live 08/03/2026, 235,383 rows** (17,859 of 17,875 raw bodies non-empty). **IS the root** for per-listing event history (price cuts, vendor per-event DOM, MLS board, `last_status_change_date`): sole source, listing-scoped. Computed `listing_dom` view STAYS the DOM root; vendor `days_after_listed` = validation column only, itself a vendor STRING ("111 days") requiring leading-digit extraction, never a raw integer. `price_change_percentage` is vendor-heterogeneous (mixes real "%" values with "$"-formatted amounts under one field) — stored as raw text, never cast to numeric. Migration `migrations/20260803_steadyapi_listing_events.sql`; parser `ingest/pipelines/listing_lifecycle/parse_listing_events.py` (idempotent TRUNCATE+re-derive, floor-guarded). Consumer: `active-listings-swfl` pack (`recent_price_cuts_count_swfl` metric, via `data_lake.listing_recent_price_cuts_stats` view, `migrations/20260803_listing_recent_price_cuts_stats.sql`) — extending the existing brain per the 08/01 listing-status-wire precedent, not a new one. Design: `docs/superpowers/specs/2026-08-03-steadyapi-listing-events-design.md`. No quality-registry contract (no county twin — nothing else we hold carries per-listing event history).
  - `steadyapi_tax_history` (← `tax_history[]`) — 🟢 **live 08/03/2026, 273,051 rows** (16,514 of 17,875 raw bodies non-empty; `(property_id, tax_year)` dedupe key, zero collisions, no cast landmines). **NOT a valuation root, ever**: assessed/market value stays `leepa_parcels`/`collier_parcels`. Role = intended home for ANNUAL TAX PAID (`median_annual_tax_swfl` metric, vendor-reported, explicitly caveated ≠ county tax bill; fixes `should_i_sell_property_tax_source`'s false premise that no root exists). Migration `migrations/20260803_steadyapi_tax_history.sql`; parser `ingest/pipelines/listing_lifecycle/parse_tax_history.py`. Consumer: `active-listings-swfl` pack, via `data_lake.listing_recent_tax_paid_stats` view (`migrations/20260803_listing_recent_tax_paid_stats.sql`). Design: `docs/superpowers/specs/2026-08-03-steadyapi-tax-history-design.md`. **Quality-registry contract (steady_tax ↔ county valuation, the third cross-source agreement family per leepa↔fdor discipline) DEFERRED** — check `steadyapi_tax_history_quality_contract_address_join_owed`: `leepa_parcels` (Lee) has no site-address column at all, and no pipeline has ever built a FDOR-side address_key normalizer; that's a real sub-project (Marco Island 0/360 precedent), not a quick join.
  - **⚠️ ALL THREE ENTRIES BELOW WERE OVERTAKEN 08/04/2026 — READ THE DECISION TABLE ROWS AT THE TOP OF THIS FILE FIRST.** A second session built a parallel VIEW (`_v` suffix) over the same raw bytes for each of the three families, at identical row counts, without knowing these tables existed. Two objects, one concept, in all three families — unresolved, check `steadyapi_three_families_table_vs_view_duplicate`. Nothing new may consume either side until the merge is decided.
  - `steadyapi_property_permits` (← `building_permits[]`) — **CORRECTION 08/04/2026: this 🔴 is STALE — the table is LIVE with 79,281 rows** (live-probed 08/04, and the 08/03 sonnet handoff already recorded it). It has no migration file in `migrations/` and no parser in the repo — see the duplicate check above. Intended root for "permits on THIS probed property" (listing scope; live 08/02: `lee_building_permits` 303 rows / `collier_building_permits` 4,975 stale 04/30 — both remain the area-wide lane, `permits_spine_thin_collier_missing` stays open). `effective_date` is a HUMAN string ("Mar 8, 2021"), not ISO.
  - **COVERAGE LAW for all three:** listing-scoped (probed listed/sold properties only, Lee-heavy by construction — book Lee 24,548 · Collier 9,142 · Hendry 1,214) — NEVER serve as a county-wide statistic. Validation joins via normalized site address, match-rate measured and reported separately (Marco Island 0/360 precedent). One-to-many: NEVER onto `listing_state`.
- SIBLING RAW LANDINGS (08/02/2026 decree — EVERY paid surface raw-lands, shipped same day): `steadyapi_search_raw` (property_id PK, latest-wins full /search item, nightly sweep) · `steadyapi_rentals_search_raw` (property_id PK, weekly sweep) · `steadyapi_price_histogram_raw` (county+captured_date) · `steadyapi_market_details_raw` (zip+captured_date; written BEFORE the content-contract gate so an aborted batch keeps its evidence) · `steadyapi_geo_trends_raw` (anchor+captured_date). All cold stores via `ingest/lib/raw_landing.py`, migration `migrations/20260802_steadyapi_raw_landings.sql` — NONE is a served root. Remaining un-landed paid surface: TS-side `/nearby-home-values` (`lib/assistant/comp-helper.ts`, live answer path — tracked in check `steadyapi_raw_land_all_paid_surfaces`). **This line under-counted until 08/19/2026:** TS-side `/property-tax-history` was un-landed too and nobody had written it down — closed that day (see SECOND WRITER above); `/nearby-home-values` is now the one that remains.
- PROVENANCE: user-facing citation = "realtor.com", never "SteadyAPI" (operator decree).
...
- DATA AVAILABLE, unpulled: field-level extraction complete as of 07/16; remaining ceiling is ENDPOINT-level — unused sibling SteadyAPI endpoints (/neighborhood-market-trends, /environment-risk, /nearby-home-values, /similar-homes, /new-construction, /property-urgency views-saves, etc.). `/neighborhood-amenities` wired 08/03/2026 — see the "Community / subdivision grain" row above, `steadyapi_neighborhoods` family. See `docs/steadyapi-research/2026-07-16-realtor-full-scope-audit.md`.
- ROUTES:
...
- **DATA PULLED 08/12/2026 (partial):** FEMA "NFIP Residential Penetration Rates" — the flat `INSURED_PENETRATION_FACTOR=0.3` NSI-proxy guess is retired; code now uses `INSURED_PENETRATION_FACTOR_BY_COUNTY` (fema-nfip-source.mts), a hardcoded snapshot of the real per-county rate pulled live from `https://www.fema.gov/api/open/v1/NfipResidentialPenetrationRates` (as of 08/03/2026): Lee 24.25%, Collier 29.55%, Hendry 4.4%. Not yet a live ingest pipeline — re-pull quarterly. **DATA AVAILABLE, still unpulled:** Policies-in-Force dataset; Community Status Book (registry L820).
- **ROUTES:**
