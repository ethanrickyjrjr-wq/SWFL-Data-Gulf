...
- **`listing_active_stats.listing_count`** 🟡 — root: **active for-sale HOME inventory** (homes-only, Lee/Collier).
  ...
  · Govern/DELETE: `listing_momentum_stats.active_listing_count` (all-types incl. ~7,300 land — a SEPARATE root if ever needed, never labeled "active listings"), `active_listings_residential_zip_stats`, raw flag-counts.
