...
- TYPED ROOTS (Step 3 of the playbook, each behind the brain-first gate) — **AUTHORITY DESIGN LOCKED 08/02/2026 (operator decree: best source per concept, validate what we already hold, listing-scope law; full design = playbook §STEP 3):**
  ...
  - `steadyapi_tax_history` (← `tax_history[]`) — 🔴 not yet built. **NOT a valuation root, ever**: assessed/market value stays `leepa_parcels`/`collier_parcels`. Role = third cross-source agreement family (steady↔county, leepa↔fdor discipline) + intended home for ANNUAL TAX PAID (no built root today; vendor tax ≠ county bill — validate before serving; fixes `should_i_sell_property_tax_source`).
