...
- STATUS: 🟡 **built, zero rows yet** — table + code shipped 08/02/2026 (`to_regclass` → non-null, migration `20260802_steadyapi_property_history_raw.sql` run live; `fetch_sold_event_raw` wired into `backfill_listed_date.py` AND all 3 of `pipeline.py`'s nightly probe call sites — departure/recheck + price-recheck). Still empty: the paid backfill run (Step 2, ~17,880 calls) hasn't fired yet, and the nightly cron is disabled (`ENGINE_ENABLED=false`) — every write path is live, but nothing has probed since deploy. Build check: `steadyapi_pth_raw_landing_before_backfill` (still open — closes on Step 2 evidence).
- ROOT: **IS the root** for raw `/property-tax-history` vendor bodies — `data_lake.steadyapi_property_history_raw (property_id text PK, address_key, county, body jsonb, fetched_at)`, idempotent `ON CONFLICT (property_id) DO UPDATE`. Cold provenance landing, NOT a consumer surface; typed roots parse OUT of it with zero paid calls.
...
- FUTURE TYPED ROOTS (🔴, Step 3 of the playbook, each behind the brain-first gate) — **AUTHORITY DESIGN LOCKED 08/02/2026 (operator decree: best source per concept, validate what we already hold, listing-scope law; full design = playbook §STEP 3):**
  - `steadyapi_listing_events` (← `property_history[]`) — **IS the intended root** for per-listing event history (price cuts, vendor per-event DOM, MLS board, `last_status_change_date`): sole source, listing-scoped. Computed `listing_dom` view STAYS the DOM root; vendor DOM = validation column only.
  - `steadyapi_tax_history` (← `tax_history[]`) — **NOT a valuation root, ever**: assessed/market value stays `leepa_parcels`/`collier_parcels`. Role = third cross-source agreement family (steady↔county, leepa↔fdor discipline) + intended home for ANNUAL TAX PAID (no built root today; vendor tax ≠ county bill — validate before serving; fixes `should_i_sell_property_tax_source`).
  - `steadyapi_property_permits` (← `building_permits[]`) — intended root for "permits on THIS probed property" (listing scope; live 08/02: `lee_building_permits` 303 rows / `collier_building_permits` 4,975 stale 04/30 — both remain the area-wide lane, `permits_spine_thin_collier_missing` stays open).
