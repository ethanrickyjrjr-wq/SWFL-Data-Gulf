...
- STATUS: 🟢 **live, 17,875 bodies** — table + code shipped 08/02/2026, paid backfill run (Step 2) completed by 08/03/2026 (newest `fetched_at` 08/03 01:11 UTC). `undated_active_sale` = 0 — the DOM backfill fully de-floored. Nightly cron still disabled (`ENGINE_ENABLED=false`, operator-owed re-enable, check `engine_enabled_kill_switch_owed`).
