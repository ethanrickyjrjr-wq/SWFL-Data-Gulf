...
| **★ Community crosswalk — READ BEFORE touching subdivisions/communities/HOA/amenities** | `docs/standards/community-crosswalk-playbook.md` — the 81-row `community_profiles` table vs 20,369 platted subdivision names is a real gap; the existing crosswalk (`refinery/lib/subdivision-aliases.mts`) is a stub (81:81, adds zero coverage); string-stemming was tried and measured near-useless (23 of 20,369 collapsed); the real fix is public-record PUD/PD boundary geometry (Lee's 1,627-polygon layer verified live 08/12/2026), not name-matching. Read this before re-deriving any of it from scratch |
