...
- ⚠️ **NOT BUILT — persistence.** The intent is that results are persisted on first fetch and read
  from our own table thereafter, so coverage compounds and the same address is never bought twice.
  **`listing_photo_enrichment` is 🔴 in data-roots and nothing writes it today**, so every build and
  every `/refresh` re-buys the same rows. Do not read this bullet as a description of live behavior.
  Tracked: `apify_enrichment_not_persisted_rebuys`.
...
| # | Failure | Guard |
|---|---|---|
| F1 | **Partial photo coverage silently ships a photo-less table.** `compsMiddle` rendered images only when `comps.every(c => thumbnails.get(...))`. Fill 4 of 6 and the reader gets ZERO photos — requirement #4 disappears with no error anywhere. | **CORRECTED 08/03/2026 — the first fix was wrong.** It made coverage gate comp *selection*, which let photo availability decide which comps ship and therefore moved `buildPriceCase`'s median, `vsSold` and position claim — the argument of a price-DEFENSE email shifting for a reason unrelated to the houses (the `claims.ts` failure class). The real fault was the all-or-nothing RENDER rule: `compsMiddle` is now **per-row**, which is the contract `comp-photos.ts` already stated ("a comp we cannot photograph ships without a picture and keeps its realtor.com link"). Selection stays nearest-first, decided before any photo is fetched. **Tests:** the price case is identical under full/partial/zero coverage; partial coverage renders the photos we DO hold. |
...
| F5 | **The description is unsourced prose in an email governed by a no-invention gate.** | It is vendor-verbatim and never model-touched: the narrator never receives it (it is not in `narratorClaims`), so it cannot be paraphrased into a claim. Provenance is the block's own `linkUrl` to the listing plus the recipe's existing collapsed `sources` tail — **not an inline attribution line**, because rules-of-engagement #1 keeps sources out of the prose. **Test:** the built description block's text is a literal substring of the source remarks. |
