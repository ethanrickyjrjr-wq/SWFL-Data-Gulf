...
- `accepted_values(col, [v1,…])` → `SELECT count(*) FROM <t> WHERE <col> IS NOT NULL AND <col> NOT IN (…)`

Pure SQL builders `build_not_null_sql / build_unique_sql / build_accepted_values_sql` return `(psycopg.sql.Composed, params)` — unit-testable without a DB. Each test runs in its own `try` / `conn.rollback()` (a missing table can't break the run, mirroring `_fetch_max_freshness`). Result per test: `{table, col, test, severity, failing_rows, status: PASS|FAIL}`.
...
- `quality_fail_<table>_<col>_<test>` — opened on error-FAIL, auto-closed when it passes again.
- `schema_drift_<table>_<col>` — opened on TYPE_CHANGED, auto-closed when the diff clears (or after `--update-baseline` re-bless lands).

`warn`-severity fails are **summary only** (no checks row) — this is why `accepted_values(source_name)` is `warn`: a new news source is an expected-to-evolve soft signal, not a tracked obligation. Auto-close scans `WHERE state='open' AND (check_key LIKE 'quality_fail_%' OR check_key LIKE 'schema_drift_%')` (parenthesized so the prefix-OR binds before the state filter) and closes any whose underlying condition cleared, exactly like the gap detector.
...
1. **SQL builders** — `build_not_null_sql` / `build_unique_sql` / `build_accepted_values_sql` emit the expected `count(*)` shape with identifiers as `Identifier` and values as bound params (assert no raw interpolation).
2. **Baseline-diff classifier** — `diff_schema(baseline, live)` returns `ADDED` / `REMOVED` / `TYPE_CHANGED` correctly; identical maps → empty; missing-baseline path.
3. **Quality-registry smoke** — `quality_registry.yaml` parses; every entry has `table`/`col`/`test`/`severity`; `accepted_values` entries carry `values`.
...
- `python -m ingest.scripts.check_data_quality --dry-run` runs **green/exit 0**, summary contains a **"Quality — value tests"** section and a **"Schema drift"** section for the four pilot tables.
