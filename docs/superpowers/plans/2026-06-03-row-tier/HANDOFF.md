...
**Sweep — ✅ BUILT 2026-06-04** (spec `sweep-spec.md`; verify against `git`): `loader.mts` 1a (polarity gate → enum-membership) + 1b (`gateVector`) shipped + the §3 drift pin is green in `grade-config-polarity.test.mts`; the sweep tool `refinery/tools/grade-config-sweep.mts` emits `sweep-output.json`. First-run tallies: **gradeable 21 · backtest_clean 11** (= the vintage audit's clean count, exact) · **moat-fuel 141** (cheapest unlock backlog) · **row-candidate 17** (R1 candidates) · **invalid-polarity 3** · needs-window 14 · unregistered 0. **Remaining = Opus semantic, NOT mechanical:** (1) **R1 row-vs-brain** confirm over the 17 row-candidates; (2) **COND 1/2 directional audit** of the 3 invalid-polarity slugs (`dbpr_notices_abt_90d`, `dbpr_releases_abt_90d` = `higher_is_bearish`; `licenses_cbc_share_swfl` = `neutral`) — **FIX-OR-REMOVE, never string-normalize** (one `checks` row each, owed — secrets were absent the build session so they were flagged in SESSION_LOG, not yet opened); (3) **C1 web-refutation before P4**. The locked contract, hardened across two LB review rounds and grounded against `loader.mts` / `predictions-log.mts`:
