...
- **Blast radius is live-inert today.** The only `gradeable:true → false` flip is `licenses_cbc_share_swfl` (`"neutral"`), whose pipeline is **not running** — no live prediction path touches it. The `higher_is_bearish` dbpr slugs were already `false` (window-blocked); tightening only changes their _reason_. Safe to ship now.
...
- **COND 1 — `higher_is_bearish`** (dbpr ABT slugs, `brain-vocabulary.json:3164,3179`). Monotonically equivalent to `lower_is_bullish`, but **string-normalizing preserves any directional error the author baked in** (the cre-swfl polarity-inversion lesson). Fix = re-derive the correct polarity from the metric's economic meaning _for its consuming brain_ (is rising ABT-enforcement actually bearish?). Inert today (window-blocked), but the wrong token must not survive into a future where `regulatory` gets a window.
