...
1. **Data revisions.** BLS (LAUS/OEWS/QCEW) and Census revise heavily. A retrodiction must use the value _as it read on the prediction date_ (the vintage), not today's revised number — else the future leaks into the past and the backtested record is a lie that collapses live. **This is R4's `vintage_policy {overwrite|append_asof}` field, now load-bearing for the moat.** Most pipelines `overwrite` today → for those slugs the as-of-then value is gone and the backtest is contaminated. Clean only where the source publishes a revision-stable archive or we retained vintages. _(Irony: the bitemporal facet we cut from the Source-Contract spine is exactly what the backtest needs — but as one job's input, not a mandatory materialization gate. "No to the spine" still holds.)_
...
3. **Survivorship.** Pre-register the full event catalog and grade ALL of it, duds included. Mining only the events that "worked" is a cherry-picked record that dies in production.

**Why this unifies with the row tier (not competes):** the Option-1 `resolveGradeConfig` sweep that partitions row/brain can, in the same pass, record per slug — (a) row vs brain, (b) the moat-fuel backlog (ungradeable _only_ for missing `direction_polarity`), and (c) **which slugs are backtestable (revision-stable / vintaged).** One sweep, three payoffs. The backward engine falls out of the sweep, provided the sweep also reads `vintage_policy`.
...
| **R6 — citation honesty (no phantom provenance)**   | 🔧 **Real gap — build then lock** | ⚠️ Verified GAP: `buildSourceCitationUrl` (`refinery/lib/citation-url.mts`) only swaps page-origin via `NEXT_PUBLIC_SITE_URL`; it does **not** branch on build-mode. A fixture/snapshot build emits a live-looking `/r/source/[table]` citation = phantom. A non-live build must not emit a live-provenance citation. Build branch + test, then lock. |
...
**Lock-now batch** (the genuine mechanizable items, all in `HANDOFF.md` Step 0): R8 hook + R4 assertion + R6 guard + CLAUDE.md C1/C2. Do **not** lock R1/R2/R3 into CLAUDE.md — that recreates the rotting-marker drift Phase 0 just cleaned out.
