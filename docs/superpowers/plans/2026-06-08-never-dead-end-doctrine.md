...
> **STATUS (2026-06-08):** Tasks **0–4 SHIPPED on `main`.** 0+1 = `a05a6c5`; 2 = `b0f408e` (slug threaded chip→popup→converse); 3+4 committed together (converse never-dead-end floor + authored-method injection + deterministic `need`-component gap-log; migration `20260608_data_requests_components.sql` applied live). **Remaining: Task 5** (span-aware action-only chips) + **Task 6** (Open-in-your-Claude link — Vendor-First verify the `claude.ai/new?q=` param in-session first). Verify against `git`, not the per-step checkboxes below.
