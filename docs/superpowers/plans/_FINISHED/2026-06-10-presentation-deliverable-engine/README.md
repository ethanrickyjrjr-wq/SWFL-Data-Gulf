...
| 2b | Frame: franchise survival | `phase-2-visuals/frame-franchise-survival__SONNET.md` | Sonnet | 2a | ✅ 2026-06-11 (local) — `FranchiseSurvivalFrame.tsx` (ranked h-bar, 4-KPI tile row, sort controls, click-to-expand detail panel, median marker, as-of caption); `franchise-survival-utils.ts` (pure adapter: prepareBrands/sortBrands/computeMedian/computeKPIs/barColor); 22 data-adapter tests pass, tsc 0. **CORRECTION 2026-06-11:** the component was built but NEVER actually added to `CHART_REGISTRY` (prior row claimed "registered" — it wasn't; it lived only in a docstring example). Now registered as `franchise-survival` (`accepts: ["ranked-categories"]`, **`fixtureOnly: true`**). Phase 3 did NOT wire it live: franchise-outcomes' `--- OUTPUT ---` emits one aggregate metric (`overall_survival_rate`) and no `detail_tables`; the per-brand ranked rows exist only in the brain's `--- SAVED FACTS ---` prose, which the thin-pipe rule forbids reading. Stays fixture-only until franchise-outcomes emits a per-brand detail_table + a `franchise-survival` `buildFrame` case land in the same (brain-first) PR. |
