...
|---|------|-------|-------------|-----|--------|-------|-----------|
| 1 | **Home services / restoration** (roof, HVAC, pool, remodel, water/mold) | Med (phone/mail, no email) | High *as leads* / Low *as newsletter* | High (buy leads $50–100 ea) | `lead-gen` | 🟡 | [01](01-home-services-restoration.md) |
| 2 | **Insurance agents** (homeowners/flood/condo) | Med (DFS list, verify email) | High (flood+storm+SIRS+values = client-open content) | High (FL crisis, retention) | `subscription` | 🟡 | _todo_ |
| 3 | **Mortgage brokers / lenders** | Med (NMLS registry) | High (rates+values+heat+affordability) | High (same as RE) | `subscription` / `lead-gen` | 🟢 (reuses engine) | _todo_ |
| 4 | **Property managers / HOA (condo)** | Low (no clean list) | High (SIRS reserve mandate = live crisis) | Med-High (mandate-driven) | `subscription` / `licensing` | 🟡 | _todo_ |
...
- [ ] **Email acquisition.** No state licensee file (DBPR/DFS/NMLS) reliably carries email. Every
      subscription/newsletter play needs an email-enrichment lane (append service, or capture via a
      signup funnel) OR a channel pivot to phone/SMS/direct-mail. This is the #1 shared blocker.
- [ ] **Verify DFS insurance-agent list** — columns, county filter, whether email is present.
- [ ] **Verify NMLS registry** — contact fields for SWFL originators.
- [ ] **Homeowner-audience list** — for lead-gen and for "content their audience opens," we need the
      parcel/homeowner side (LeePA/CollierPA) wired as an audience, not just a data source.
- [ ] **Content that a non-RE audience opens weekly** — most lake metrics are B2B analytics, not
      consumer newsletter fodder. Each subscription play needs a proven "reader actually opens this"
      content recipe, not just a data dump.
...
- `01-home-services-restoration.md` — deep dive: the closest-to-payable *new* vertical.
- _add one `NN-<vertical>.md` per play as we go deep._
