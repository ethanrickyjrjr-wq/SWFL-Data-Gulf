...
- ~~`lee_no_second_parcel_source` — Do NOT add a second Lee parcel ingest - LeePA live 548,330; FDOR centroid CO_NO=46 (556,100) is a duplicate. Lee=LeePA, full stop. Kill any in-flight Lee-from-FDOR ingest~~ **SUPERSEDED 07/18/2026 — DO NOT ACT ON THIS LINE.** The operator ratified **KEEP BOTH** (`docs/handoff/2026-07-18-parcel-consolidation.md:24`): `lee_parcels` (FDOR CO_NO=46) LANDED 07/18/2026 with **556,083 parcels / 104 cols** and is live with the `lee_parcels_summary` view, while `leepa_parcels` stays permanently as the **appraiser cross-check** (it carries `building_value`, deed instrument, full sale date and the `folioid` key that FDOR lacks). Obeying the struck line today would delete a live, ratified root. Current authority: `docs/standards/data-roots.md` §PARCELS. Verified against the tree 08/11/2026; this key is **not** an open row in the `checks` ledger.
