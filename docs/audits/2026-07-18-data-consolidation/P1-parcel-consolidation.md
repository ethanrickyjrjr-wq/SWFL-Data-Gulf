# P1 — Lee Parcel Consolidation + Guarded Deletion Plan

Stream: P1-parcel-consolidation. Written 2026-07-18. Read-only analysis; NO repo files edited, NO
DROP/DELETE run. Every claim is backed by a live query (Postgres lake via `mcp__supabase__execute_sql`)
or a `file:line` citation. Recommendations that change system shape are tagged **[NEEDS-SIGN-OFF]**
(operator C1/C2 ratification, never asserted here as "the authority").
...
|---|---|---|
| `soh_cap` | `jv_hmstd − av_hmstd` | `jv_hmstd`/`av_hmstd` both **100% populated** (290,973/290,973) in collier; 107,029 homesteaded-capped parcels (`jv_hmstd > av_hmstd`), mean derived cap ≈ $386,042. Structurally sound; parcel-level reconciliation to LeePA's own `soh_cap` needs a `folioid↔parcel_id` crosswalk that does not exist yet. |
...
- **Read by:** `refinery/sources/leepa-value-source.mts:33,165-170` (`SALES_YEARLY_VIEW`).
