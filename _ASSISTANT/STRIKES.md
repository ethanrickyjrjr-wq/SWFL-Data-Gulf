...
- strike: 08/11 nightly-chain.yml `listings:` job deleted, its trailing `secrets: inherit` left orphaned (cb803b1c) → GitHub rejects the file, 2 runs with ZERO jobs. PyYAML parses it fine (glues the orphan onto `guard:`), so a local yaml check is green while the real gate is red. SAME MECHANISM, NEW SURFACE: the BUILT guard above only covers test-mock leaks; nothing validates workflow YAML against the real parser. Guard owed: actionlint over changed `.github/workflows/*.yml` in the existing pre-push gate. **GUARD SHIPPED 08/12/2026** — see the strike below.
- strike: 08/12 graphify-republish.yml — swapping 26 install steps to a composite action left `working-directory: brain-platform` sitting on what was now a `uses:` step. That key is legal ONLY on `run:` steps, so GitHub rejected the whole file: zero jobs, `gh run view --log-failed` → "log not found", run titled by FILE PATH instead of `name:`. A strict `Bun.YAML.parse` over all 27 changed files had said CLEAN — same lesson as the 08/11 strike, learned again: valid YAML ≠ valid workflow. **GUARD BUILT, red-first: `.github/scripts/workflow-step-shape.test.mjs`** — real YAML parse of every workflow AND local action, the run-only-key rule (`run`/`shell`/`working-directory` on a `uses:` step), and local `uses: ./…` path existence including the `checkout path:` subdirectory case. Proven by re-introducing the exact bug and watching it fail, then reverting. Wired into pre-push Gate 1.6 so it fires BEFORE the push, not in CI after. Chose this over actionlint (the guard the 08/11 strike asked for): no new binary, ~200ms, and it names the two failures we have actually had — RULE 11.
...
## shape: stale-claim-blocks-work-and-the-wait-does-not-detect-it
guard: OWED — `claim wait` EXITED 0 TWICE without granting the claim, and `claim waits` reported "No sessions are waiting" while two of this session's waits were supposedly queued. A claim whose holder already committed and moved on blocks every other session indefinitely, and the tool offers no way to tell a live hold from a dead one. **The safe-override test, use it before any release:** (1) `git status --short -- <file>` shows NO uncommitted work, (2) `git log -1 -- <file>` shows the holder already committed, (3) `claim waits` shows nothing genuinely queued. All three true = stale, nothing to clobber, release is safe and say so. Mechanism owed: stale-claim detection (auto-expire on holder-commit, or flag a claim whose file has been clean since before the claim's own age).
- strike: 08/12 `_ASSISTANT/STRIKES.md` held by three sessions across one session; the last held it 24 min AFTER committing its work (`b2edd782`), file clean the whole time. Two backgrounded waits exited 0 and the edits were still denied. Cost: the strike lines this very registry exists to hold were blocked for the entire session and nearly shipped as "owed" instead of written.
