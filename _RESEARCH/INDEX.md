...
**agent-behavior/** — how the agent should steer, drift, schedule, self-check (18)
- `2026-09-21-jev-typesafe-decision-model-evaluation.md` — **Jev** (TypeSafe AI's hosted, closed, PAID "System One" decision model: structured state in → typed Choice/Score/yes-no out, no text; vendor-stated $0.042/M input, 70–500 ms) via a 0xRicker "Jev Engineering" thread. "193x faster / 444x cheaper" is the vendor's own four-workflow harness. Vendor-published weak spots: numeric precision, DATE comparison, large state, adversarial state. **Verdict: do not adopt** — routing already answered 07/30 (one provider), our decision layer is already code at $0, product API spend was $50.12/30d so the saving is cents. Shape to keep: creates text → LLM; semantic pick/score → small classifier (local box first); exact rule → code.
...
**data-and-ingest/** — sources, backfills, pipeline findings (24)
- `2026-09-18-first-builds-integration-review.md` — Independent Terra/Sol review, manifest/unit/revision/coverage repairs, 23 Python + 20 Bun passing tests, actual deterministic 91-month packet, tools readiness, FDOT Guest CAPTCHA blocker, and bounded S2/T3 next assignments. Changes remain local worktree work; predictive value and unattended operation are unproved.
- `2026-09-18-data-access-and-outcome-feasibility.md` — Live source qualification for the Terra/Sol proposal: current July RSW PDF parses with existing code; Census BPS county rows fetched for Jan 2010 and Jul 2026; tax-sales archive works but both probed current paths 404; FDOT continuous local coverage remains unproven. Separates commercial panels, obtainable observations, and scoreable outcomes. Plan: `docs/superpowers/plans/2026-09-18-data-first-intelligence-rebuild.md`.
- `2026-09-15-satellite-parking-lot-traffic-vendor-evaluation.md` — Ricky asked about planet.com for tracking parking-lot occupancy/traffic. Live-crawled Planet (SkySat 50cm tasking = the tier that resolves cars, enterprise-quote-only pricing) and Nearmap (4cm aerial but only 3x/year refresh — wrong cadence for traffic tracking regardless of price). **Verdict: DO NOT ADOPT now** — both are hedge-fund/insurer-scale spend, a rounding error of our volume. If revisited, go through a pay-per-task marketplace (SkyFi, unverified) on named Lee/Collier parcels, not a subscription.
...
- `2026-08-29-mineru-evaluation.md` — operator asked "is this better than what we have?" on opendatalab/MinerU. **NO — nothing for it to be better at.** Our whole PDF lane is PyMuPDF + pdfplumber on two TEXT-PDF feeds (marketbeat_pdf, rsw_airport_monthly) that already clear every page; zero scanned PDFs exist in the repo; MarketBeat output is gated dead anyway. Live LICENSE.md (3.1.0+): left AGPLv3 for Apache-2.0 + an **online-service attribution clause** (trap shape #7 of the 08/27 sweep, confirmed from the license itself). 20GB disk / 16GB RAM floor / `ray` / Python ≤3.12 on Windows to replace two pip packages. Re-open only if a scanned corpus ever lands — then docling vs MinerU-Pro bake-off.
