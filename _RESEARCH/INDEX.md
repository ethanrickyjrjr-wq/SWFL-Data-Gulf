...
- `2026-09-15-max-subscription-vs-api-key-for-pipeline-calls-evaluation.md` — can the Max plan pay for pipeline LLM calls instead of the API key? `LLM_PROVIDER` is read by NOTHING here; the factory has no provider seam (`anthropic.mts:15-17` = key present → metered, key absent → mock). The mechanism EXISTS (`claude setup-token` → `CLAUDE_CODE_OAUTH_TOKEN`, vendor's own words "for CI pipelines, scripts") but `ANTHROPIC_API_KEY` OUTRANKS it in the documented precedence, so a naive attempt silently bills the API anyway. **Verdict: do not adopt for the pipeline** — the Agent SDK terms bar offering claude.ai rate limits "for their products", it spends the operator's own coding window, and MEASURED spend is only $6.03/30d ($81.42 lifetime; refinery synthesis+triage = $12.30 TOTAL, nothing since 08/10). 🔴 Found live: `RATES` has no `claude-sonnet-5`/`claude-opus-5` row, so 58 sonnet-5 calls logged at $0.00 and are invisible to the spend caps.
