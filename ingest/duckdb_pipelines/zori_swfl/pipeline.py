...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="zori-swfl ingest pipeline.")
    ...
    try:
        if args.dry_run:
            with tempfile.TemporaryDirectory() as tmp:
                print("zori-swfl: --dry-run, writing to a temp dir; no S3, no inventory row.")
                run(target=f"{tmp}/zori_swfl.parquet")
            return
        run()
