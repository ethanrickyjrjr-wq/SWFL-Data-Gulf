"""Zillow ZHVI SWFL home-value-index → Parquet in Tier 1 Storage.

Run with: python -m ingest.duckdb_pipelines.zhvi_swfl.pipeline

What it does:
  1. Stream-downloads the ZIP-level all-homes middle-tier smoothed ZHVI CSV from
     files.zillowstatic.com to a local temp file.
  2. DuckDB filters to FL + four SWFL MSAs, UNPIVOTs the wide month columns
     to long form (zip × period_end × home_value), and writes ZSTD Parquet
     to s3://lake-tier1/market/zhvi_swfl.parquet.
  3. Upserts one row in data_lake._tier1_inventory.

The temp-file step mirrors zori_swfl — keeps S3-write credentials cleanly
isolated from the public source-bucket fetch.

Update schedule: Zillow Research refreshes monthly (~3rd week of the month
for the prior month). Cron target: 22nd of each month.
"""
...
def run(*, target: str = PARQUET_TARGET, source_csv: str | None = None) -> None:
    ...
    print("zhvi-swfl: starting ingest")
    print(f"  source: {source_csv or ZHVI_CSV_URL}")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        if source_csv:
            ...
            csv_path = Path(tmp_dir) / "zhvi_zip_all_homes.csv"
            _download_source(ZHVI_CSV_URL, csv_path)
        ...
        #   melted: UNPIVOT every non-metadata column into (period_end_str, home_value).
        #   typed:  cast + clean.
        con.execute(f"""
            CREATE TABLE zhvi_swfl AS
            WITH wide AS (
                SELECT *
            ...
            melted AS (
                ...
                INTO NAME period_end_str VALUE home_value_raw
            )
            SELECT
                CAST(RegionName AS VARCHAR)        AS zip_code,
                CAST(period_end_str AS DATE)       AS period_end,
                CAST(home_value_raw AS DOUBLE)     AS home_value,
                Metro                              AS metro,
                CountyName                         AS county_name,
                City                               AS city,
                '{ingested_at}'                    AS ingested_at
            FROM melted
            WHERE home_value_raw IS NOT NULL
        """)

        row_count = con.execute("SELECT COUNT(*) FROM zhvi_swfl").fetchone()[0]
        zip_count = con.execute(
            "SELECT COUNT(DISTINCT zip_code) FROM zhvi_swfl"
        ).fetchone()[0]
        metro_count = con.execute(
            "SELECT COUNT(DISTINCT metro) FROM zhvi_swfl"
        ).fetchone()[0]
        date_range = con.execute(
            "SELECT MIN(period_end), MAX(period_end) FROM zhvi_swfl"
        ).fetchone()
        print(
        ...
        con.execute(
            f"COPY zhvi_swfl TO '{target}' (FORMAT PARQUET, COMPRESSION ZSTD)"
        )
