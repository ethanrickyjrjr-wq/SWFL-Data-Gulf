"""Unit tests for the zhvi_swfl dlt resource.
...
def test_resource_is_decorated_with_merge_on_composite_pk():
    ...
    bound = zhvi_swfl_resource(rows=FAKE_ROWS)
...
def test_resource_emits_all_columns_and_metadata():
    ...
    rows = list(zhvi_swfl_resource(rows=FAKE_ROWS))
    assert len(rows) == 2
    ...
    assert first["_ingest_metadata"]["source"] == "zillow_zhvi_research"
    assert first["_ingest_metadata"]["tier1_bucket"] == "lake-tier1"
    assert first["_ingest_metadata"]["tier1_path"] == "market/zhvi_swfl.parquet"
...
def test_resource_handles_missing_optional_metadata():
    ...
    rows = list(zhvi_swfl_resource(rows=minimal))
...
def test_read_tier1_parquet_local_path(tmp_path):
    ...
    import duckdb
    from ingest.pipelines.zhvi_swfl.resources import read_tier1_parquet
    ...
    con.execute(
        ...
        SELECT
            ...
            CAST(612000.0 AS DOUBLE) AS home_value,
    ...
    assert rows[0]["home_value"] == pytest.approx(612000.0)
