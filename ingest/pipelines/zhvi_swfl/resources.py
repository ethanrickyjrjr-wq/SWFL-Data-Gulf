"""Zillow ZHVI SWFL dlt resource — Tier 1 Parquet → Tier 2 Postgres merge.

Reads the Parquet that ingest.duckdb_pipelines.zhvi_swfl wrote and yields
...
from ingest.duckdb_pipelines.zhvi_swfl.constants import (
    BUCKET,
    PARQUET_PATH,
    PARQUET_TARGET,
)
...
def read_tier1_parquet(parquet_path: str | None = None) -> Iterable[dict]:
    """Read the Tier 1 ZHVI Parquet and yield row dicts.

    Args:
        parquet_path: Optional override for tests. Defaults to PARQUET_TARGET
            (s3://lake-tier1/market/zhvi_swfl.parquet).
    """
...
def zhvi_swfl_resource(
    rows: Optional[Iterable[dict]] = None,
    parquet_path: Optional[str] = None,
):
    """Emit ZHVI rows for Tier 2 merge.
