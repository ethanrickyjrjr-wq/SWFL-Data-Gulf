"""Redfin Collier County market-tracker ingest entry point.
...
from .resources import ingest_redfin_collier, iter_collier_rows


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Redfin Collier County market-tracker ingest.")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Download + filter only; print Collier row count + sample; skip dlt write.",
    )
    args = parser.parse_args(argv)

    if args.dry_run:
        rows = list(iter_collier_rows(REDFIN_COUNTY_TRACKER_URL))
        print(f"redfin_collier dry-run: {len(rows)} Collier County, FL rows")
        if rows:
