...
API, no scraping). Same URL and format as redfin_collier — one file, two
county filters.
...
55d content-freshness guard since 08/18/2026 — issue #183). Never point back at
...
TYPE "County", REGION NAME "Lee County, FL" present, newest PERIOD END
2026-08-31.
"""
...
# Exact REGION NAME string for Lee County in the county tracker (verified live).
...
# month back to 2012). The registry's `expected_rows_min: 594` guards the
# CUMULATIVE count_table, which a merge write never shrinks even if this run
# yields zero new Lee rows — it would stay "healthy" on nothing but old rows
# forever. This floor catches THIS pull going quiet.
MIN_ROWS = 150
