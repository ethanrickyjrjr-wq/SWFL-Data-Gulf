"""Stream the free Redfin county market tracker, keep Lee County rows,
...
def _row_from_cells(cells: list[str], idx: dict[str, int]) -> dict | None:
    """Build one kept row if REGION TYPE is County and REGION NAME is Lee."""
    ...
    if type_i is not None and type_i < len(cells) and cells[type_i].strip() != "County":
        return None
    if cells[region_i].strip() != LEE_REGION:
        return None
...
def iter_lee_rows(url: str = REDFIN_COUNTY_TRACKER_URL) -> Iterator[dict]:
    """Yield Lee County, FL rows from the CSV county tracker as dicts.

    Streams line by line; a cheap substring gate skips the ~99.7% of lines
    that aren't Lee before any CSV parsing (the parsed-cell check in
    ...
    if have_header and final and LEE_REGION in final:
...
def ingest_redfin_lee(url: str = REDFIN_COUNTY_TRACKER_URL) -> int:
    ...
    Lee is a single county (~hundreds of rows across periods), so a single
    ...
    if len(rows) < MIN_ROWS:
        ...
        raise VolumeGuardError(
            f"redfin_lee: returned {len(rows)} rows, below MIN_ROWS={MIN_ROWS} "
