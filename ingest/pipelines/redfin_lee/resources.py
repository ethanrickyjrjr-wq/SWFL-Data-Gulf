"""Stream the free Redfin county market tracker, keep Lee County FL rows,
and merge them into data_lake.redfin_lee_market (Tier 2).
...
The PK is stable across Redfin refreshes — each monthly update adds new rows
or revises existing ones; old rows are NOT regenerated with different keys.
This means the pipeline is idempotent: running it again never re-ingests clean
data from scratch, it only upserts what changed.
"""
...
from .constants import LEE_REGION, REDFIN_COUNTY_TRACKER_URL
...
# Tier-2 column hints — pin the Lee market row to explicit dlt types so the
...
def iter_lee_rows(url: str = REDFIN_COUNTY_TRACKER_URL) -> Iterator[dict]:
    """Yield Lee County, FL rows from the gzipped county tracker as dicts.

    Streams + decompresses incrementally; a cheap substring gate skips the
    ~99.9% of lines that aren't Lee before any tab-splitting.
    """
    ...
    for chunk in resp.iter_content(1 << 20):
        ...
        for line in lines:
            ...
            if region_i is None or region_i >= len(cells):
                continue
            if _unquote(cells[region_i]) != LEE_REGION:
                continue
            row = {}
    ...
    if have_header and pending and LEE_REGION in pending:
        cells = pending.split("\t")
        region_i = idx.get("REGION")
        if region_i is not None and region_i < len(cells) and _unquote(cells[region_i]) == LEE_REGION:
            row = {}
            for src, dst in _KEEP.items():
                i = idx.get(src)
                row[dst] = _coerce(dst, cells[i]) if (i is not None and i < len(cells)) else None
...
def _make_resource(rows: list[dict]):
    ...
    return redfin_lee_rows


def ingest_redfin_lee(url: str = REDFIN_COUNTY_TRACKER_URL) -> int:
    """Download + filter + merge Lee rows into data_lake.redfin_lee_market.

    Lee is a single county (~hundreds of rows across periods/property types),
    ...
    if not rows:
        # An empty pull (Redfin renamed LEE_REGION or moved the URL) is a REAL failure, not a
        # green no-op. Raise so the cron goes red instead of exiting 0 with stale data narrated live.
        raise VolumeGuardError(
            "redfin_lee: returned 0 rows — no Lee County rows found (check LEE_REGION filter / URL)"
        )
    ...
    assert_content_fresh(newest_period_end, 55, label="redfin_lee")
    pipeline = dlt.pipeline(
        pipeline_name="redfin_lee",
        destination="postgres",
        dataset_name="data_lake",
    )
    load_info = pipeline.run(_make_resource(rows)())
    load_info.raise_on_failed_jobs()
    print(f"redfin_lee: merged {len(rows)} Lee County rows into data_lake.redfin_lee_market")
    return len(rows)
