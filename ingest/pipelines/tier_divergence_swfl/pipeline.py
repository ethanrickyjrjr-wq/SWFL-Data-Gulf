"""Zillow ZHVI tier-divergence SWFL Tier 2 loader entry point.

Run with: python -m ingest.pipelines.tier_divergence_swfl.pipeline

Reads the Tier 1 Parquet at s3://lake-tier1/market/tier_divergence_swfl.parquet
via DuckDB and merges into data_lake.tier_divergence_swfl on
(zip_code, period_end). Idempotent — re-running against an unchanged Parquet is a
no-op.

Chained from npm: `npm run ingest:tier-divergence-swfl` runs the Tier 1 DuckDB
pipeline first (writes the Parquet) and then this script (merges to Postgres).
"""
...
from .resources import tier_divergence_swfl_resource

ZHVI_PARQUET_ID = "lake-tier1/market/tier_divergence_swfl.parquet"
...
def _ensure_tier1_fresh() -> None:
    ...
    if vintage < date.today() - timedelta(days=1):
        sys.exit(
            ...
            f"Run `gh workflow run tier-divergence-tier1-monthly.yml -f dry_run=false` first."
        )
...
def main() -> None:
    ...
    try:
        if args.dry_run:
            rows = list(tier_divergence_swfl_resource(parquet_path=args.parquet_path))
            print(f"tier_divergence_swfl dry-run: {len(rows)} rows")
            if rows:
