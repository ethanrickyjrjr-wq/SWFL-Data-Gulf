"""Zillow ZHVI tier-divergence SWFL dlt resource — Tier 1 Parquet → Tier 2 Postgres merge.

Reads the Parquet that ingest.duckdb_pipelines.tier_divergence_swfl wrote and
yields typed dicts. The `rows` parameter lets tests inject rows directly without
touching Parquet or S3.

The Tier 1 Parquet is wide-on-tier, long-on-month: one row per (zip_code,
period_end) carrying both top_tier_value and bottom_tier_value (either may be
NULL — single-tier history is kept upstream).
...
from ingest.duckdb_pipelines.tier_divergence_swfl.constants import (
    BUCKET,
    PARQUET_PATH,
    PARQUET_TARGET,
)
...
def _load_env_local() -> None:
    ...
    pipeline. Needed because `npm run ingest:tier-divergence-swfl` chains two
    python subprocesses; the second one starts with a fresh environment and does
...
def read_tier1_parquet(parquet_path: str | None = None) -> Iterable[dict]:
    """Read the Tier 1 tier-divergence Parquet and yield row dicts.

    Args:
        parquet_path: Optional override for tests. Defaults to PARQUET_TARGET
            (s3://lake-tier1/market/tier_divergence_swfl.parquet).
    """
...
@dlt.resource(
    name="tier_divergence_swfl",
    primary_key=["zip_code", "period_end"],
    write_disposition="merge",
)
def tier_divergence_swfl_resource(
    rows: Optional[Iterable[dict]] = None,
    parquet_path: Optional[str] = None,
):
    """Emit tier-divergence rows for Tier 2 merge.
    ...
    for r in rows:
        yield {
            ...
            "top_tier_value":    r.get("top_tier_value"),
            "bottom_tier_value": r.get("bottom_tier_value"),
            "metro":             r.get("metro"),
            "county_name":       r.get("county_name"),
            "city":              r.get("city"),
            "ingested_at":       r.get("ingested_at"),
            "_ingest_metadata": {
                "source":   "zillow_zhvi_tiers",
                "tier1_bucket": BUCKET,
