"""supercrawl — enhanced in-process crawl layer over crawl4ai_client.py.

The old ingest/lib/crawl_client.py is the STABLE BASE — untouched, the workhorse for every
shipped pipeline. supercrawl is the ENHANCED SURFACE: same crawl4ai 0.9.0 in-process SDK, plus
...
400-rejects js_code/proxy/cookies over the network). supercrawl is in-process SDK, like the base.
...
from ingest.lib.crawl_client import Crawl4aiError  # re-export the shared error type
