...
| **★ Data inventory — what we have, cadence, size** | `docs/standards/data-inventory.md` — source-by-source: live row count + byte size + cron status (live/parked/dispatch-only) for every ingest pipeline, plus what's free and confirmed not yet pulled. Different angle from data-roots.md (concept→root); this one is source→size/schedule |
