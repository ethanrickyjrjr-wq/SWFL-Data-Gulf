...
- Claim: the close cell PREFILLS from the last list price we hold, verbatim, and carries no
  ...
  Test: just-sold.test.ts > "F6 · NO recorded sale → the last list price PREFILLS it, with NO sold-date kicker"
  Test: just-sold.test.ts > "F9 · the prefill is our list price VERBATIM — never reformatted"
- Claim: nothing held at all is an OPEN SLOT, never a zero
  Test: just-sold.test.ts > "F8 · nothing held at all → an OPEN SLOT, never a zero"
- Claim: a prefill never reaches a DERIVED cell (`List-to-Sale`, `List Price`, `$/Sq Ft`) or a footnote
  Test: just-sold.test.ts > "F1/F2/F3 · with a prefill in the hero, the strip's derived cells stay OPEN"
  Test: just-sold.test.ts > "F2 · the footnote claims nothing when nothing was computed"
...
- Claim: a prefill is never a chart bar — the comps chart anchors on a RECORDED close or is dropped
  Test: just-sold.test.ts > "F4 · a prefill is NEVER a bar"
...
- Claim: with no recorded close the narrator may name no sale price, and never restates the prefill
  Test: just-sold.test.ts > "F5 · with no recorded close the narrator is FORBIDDEN to name a sale price"
...
- Claim: no paid sold-price lane (Apify / property-tax) is wired into this recipe — SUSPENDED by
  ...
  Test: just-sold.test.ts > "this recipe reaches for no paid sold-price lane"
- Claim: the seller's for-sale description does NOT ship on a sold email (a pitch is stale once the
  ...
  Test: just-sold.test.ts > "no `description:` is handed to the chrome — the pitch is stale once it closes"
