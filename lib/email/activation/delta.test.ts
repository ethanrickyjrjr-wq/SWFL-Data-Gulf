...
function metric(over: Partial<SnapshotMetric> & Pick<SnapshotMetric, "key" | "value">): SnapshotMetric {
  return { label: over.key, ...over };
}

function line(over: Partial<SnapshotLine> & Pick<SnapshotLine, "brain_id" | "fingerprint">): SnapshotLine {
...
describe("computeReportDelta — metric moves", () => {
  it("reports a numeric rise with delta + direction + favorability", () => {
    const prev = snap({
      metrics: [metric({ key: "housing.median_sale_price", label: "Median sale price", value: 400000, direction: "higher_is_better" })],
    });
    const current = snap({
      freshness_token: "SWFL-7421-v6-20260613",
      metrics: [metric({ key: "housing.median_sale_price", label: "Median sale price", value: 412000, direction: "higher_is_better" })],
    });
  ...
  it("marks a rise unfavorable when lower_is_better (e.g. days on market)", () => {
    const prev = snap({ metrics: [metric({ key: "housing.dom", value: 30, direction: "lower_is_better" })] });
    const current = snap({ metrics: [metric({ key: "housing.dom", value: 45, direction: "lower_is_better" })] });
  ...
  it("ignores a metric that vanished from the current assembly (can't claim it)", () => {
    const prev = snap({ metrics: [metric({ key: "gone", value: 1 }), metric({ key: "kept", value: 2 })] });
...
describe("computeReportDelta — signals (real time-grain only)", () => {
  it("reports a changed daily city-pulse line as a signal", () => {
    ...
    const current = snap({ lines: [line({ brain_id: "city-pulse-swfl", fingerprint: "new story" })] });
    const d = computeReportDelta(prev, current);
  ...
  it("does not flag a deltable line whose substance is unchanged", () => {
    ...
    const current = snap({ lines: [line({ brain_id: "permits-swfl", fingerprint: "12 permits" })] });
    expect(computeReportDelta(prev, current).signal_changes).toHaveLength(0);
  });
