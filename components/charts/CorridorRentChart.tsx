...
export function CorridorRentChart({
  ...
  const sortedData = useMemo<RentChartRow[]>(() => {
    return [...data]
      .filter((c): c is RentChartRow => c.nnn_asking_rent_per_sqft != null && c.vacancy_pct != null)
  ...
  useEffect(() => {
    ...
    const barColors = sortedData.map((_, index) => {
      ...
      const factor =
        sortedData.length > 1 ? (sortedData.length - 1 - index) / (sortedData.length - 1) : 1;
      ...
      series: [
        {
          ...
          itemStyle: {
            ...
            color: (params: unknown) => barColors[(params as { dataIndex: number }).dataIndex],
          },
          emphasis: {
            itemStyle: {
              shadowBlur: 10,
    ...
    return (
      ...
      >
        ...
        <p className="text-slate-400 text-sm max-w-sm">
          Please supply a populated roster of Corridor entries to render this visualization.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-400 mt-0.5">
          Southwest Florida’s primary commercial highways ranked from high to low. Hover columns for
          detail or click to analyze.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        ...
        <div className="lg:col-span-2 relative flex flex-col justify-between bg-slate-950/40 rounded-xl border border-slate-800/50 p-4 min-h-[350px] sm:min-h-[460px]">
          ...
          <div className="text-[10px] text-slate-500 font-mono mt-1 flex items-start gap-1.5 px-1 bg-slate-900/30 py-1.5 rounded">
            ...
            <span>
              Interactive bounds: Graph is fully responsive. Bar heights represent asking NNN rate.
            </span>
          </div>
        </div>
            ...
            {selectedCorridor ? (
              <div className="space-y-5" id={`disclosure-card-${selectedCorridor.id}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-900/40 p-3 rounded-lg border border-slate-800/30">
                    <p className="text-xs text-slate-400 font-mono">Permits Sample (n)</p>
                    <p className="text-sm font-bold text-slate-200 mt-1">
                ...
                {selectedCorridor.permits ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-slate-500">
                      permits-swfl is Lee County only. Collier permit coverage waits on a Collier
                      permits pack — see docs/data-coverage.md.
                    </p>
                  </div>
                )}
              </div>
