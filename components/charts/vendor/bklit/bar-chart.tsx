...
export interface BarChartProps {
  ...
  status?: ChartStatus;
  /** FORK (not upstream): fixed pixel size that bypasses `@visx/responsive`'s
   *  `ParentSize` (which needs a real ResizeObserver and renders 0×0 under
   *  `ReactDOMServer.renderToStaticMarkup`). When set, also forces the reveal
   *  animation to its finished state (no effect ever runs in a single-pass SSR
   *  render). Only the static/email render bridge passes this — see
   *  `render-static.tsx` and NOTICE.md. */
...
interface ChartInnerProps {
  ...
  onPhaseChange?: (phase: ChartPhase) => void;
  status: ChartStatus;
  /** FORK (not upstream): skip the grow-in reveal and render bars at final
   *  geometry immediately — required for a single-pass SSR render (email PNG),
   *  where no effect ever runs to flip `isLoaded` after the reveal timer. See
   *  NOTICE.md. */
...
export function BarChart({
  ...
  if (staticSize) {
    return (
      <div className={cn("relative w-full overflow-visible", className)} style={{ aspectRatio }}>
        <ChartInner
