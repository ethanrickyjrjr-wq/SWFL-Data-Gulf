...
import { Brain, CheckCircle2, ShieldCheck, RefreshCw } from "lucide-react";
...
export function RadialConfidenceGauge({
  ...
  if (loading) {
    return (
      <div
        className={`p-5 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between gap-4 animate-pulse h-40 ${className}`}
      >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
        ...
        <div className="space-y-3 text-center sm:text-left flex-1">
          ...
          <div className="space-y-1">
            <h3 className="text-lg font-bold tracking-tight text-white flex items-center justify-center sm:justify-start gap-1.5">
              ...
              >
                <RefreshCw
                  className={`h-3 w-3 ${isInteracting ? "animate-spin text-teal-400" : ""}`}
                />
              </button>
            </h3>
            <p className="text-xs text-slate-400 leading-relaxed font-sans max-w-sm">
              Our appraisal neural network cross-references actual sales with incoming commercial
              building permits to compute predictive reliability indexes.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 text-[10px] font-mono text-slate-500 pt-1">
            <span className="flex items-center gap-1 text-teal-400">
              <ShieldCheck className="h-3 w-3 text-teal-400" /> High Integrity
            </span>
            <span>•</span>
            <span className="flex items-center gap-1 text-slate-400">
              <CheckCircle2 className="h-3 w-3 text-emerald-400" /> 10-Fold Verified
            </span>
          </div>
        </div>
        ...
        <div className="relative flex-none flex items-center justify-center">
          ...
          <div className="absolute flex flex-col items-center justify-center text-center">
            <span
              className={`text-3xl font-extrabold tracking-tighter ${getTextColor(animatedValue)} font-mono tabular-nums leading-none`}
            >
              {animatedValue}%
            </span>
